...
global  Np Ns w_ini w_end gmax c1 c2 w_ini2 w_end2 FESMAX M repeat;
 ...
 for i=1:Ns
     [S(i,Np+2),s1,~]=predict(model,S(i,1:Np));
     S(i,Np+2)=S(i,Np+2)+(w_end2+(FESMAX-FES)*w_ini2/FESMAX+repeat(1,1))*s1;
...
S_next=zeros(Ns,Np+3) ;
 for g=1:gmax
     ...
     for i=1:30
        ...
        [S_next(i,Np+2),s1,~]=predict(model,S_next(i,1:Np));
        S_next(i,Np+2)=S_next(i,Np+2)+(w_end2+(FESMAX-FES)*w_ini2/FESMAX+repeat(1,1))*s1;
        if S_next(i,Np+2)>worst_on_S11(Np+2)
            worst_on_S11=S_next(i,:);
        end
        if S_next(i,Np+2)>worsts_on_S11(i,Np+2)
